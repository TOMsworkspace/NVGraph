...
/* ===================================================================== */
KNOB<string> KnobOutputFile(KNOB_MODE_WRITEONCE,  "pintool", "o", "nvgraph.trace", 
        "specify file name for NVGraph tracer output");
...
void MemoryRead(VOID* addr, UINT32 index)
{
    if(!tracing_on) return;
...
{
    //skip the scattered memory access instructions.
    //if(INS_HasScatteredMemoryAccess(ins)) return;

    //skip the vgather/vscatter 
    if(INS_IsVgather(ins) || INS_IsVscatter(ins)) return;
    ...
    for (UINT32 memOp = 0; memOp < memOperands; memOp++) 
    {
        if (INS_MemoryOperandIsRead(ins, memOp)) 
        {
            ...
            INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)MemoryRead,
                IARG_MEMORYOP_EA, memOp, IARG_UINT32, memOp,IARG_END);
...
int main(int argc, char *argv[])
{
    ...
    //cerr <<  "===============================================" << endl;
    //cerr <<  "This application is instrumented by the NVGraph Trace Generator" << endl;
