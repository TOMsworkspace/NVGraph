...
if [ "$#" -ne 4 ]; then
    ...
    #echo "Usage: ./build_nvgraph.sh [branch_pred] [l1d_pref] [l2c_pref] [llc_pref] [llc_repl] [num_core]"
    echo "Usage: ./build_nvgraph.sh [l1d_pref] [l2c_pref] [llc_pref] [num_core]"
    exit 1
fi

# NVGraph configuration
...
if [ "$NUM_CORE" -gt "1" ]; then
    echo "Building multi-core NVGraph..."
    ...
    if [ "$NUM_CORE" -lt "1" ]; then
        ...
        echo "Building single-core NVGraph..."
    fi
fi
echo
...
# Sanity check
echo ""
if [ ! -f bin/NVGraph ]; then
    echo "${BOLD}NVGraph build FAILED!"
    echo ""
    exit 1
fi

echo "${BOLD}NVGraph is successfully built"
...
echo "Cores: ${NUM_CORE}"
BINARY_NAME="NVGraph-${BRANCH}-${L1D_PREFETCHER}-${L2C_PREFETCHER}-${LLC_PREFETCHER}-${LLC_REPLACEMENT}-${NUM_CORE}core"
echo "Binary: bin/${BINARY_NAME}"
echo ""
mv bin/NVGraph bin/${BINARY_NAME}
