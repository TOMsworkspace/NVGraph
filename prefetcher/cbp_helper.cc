...
namespace knob
{
	...
	extern uint32_t cbp_state_type;
	extern uint32_t cbp_max_states;
	extern uint32_t cbp_state_hash_type;
	extern bool 	cbp_enable_pt_address_compression;
	extern uint32_t cbp_pt_address_hash_type;
	extern uint32_t cbp_pt_address_hashed_bits;
	extern bool     cbp_access_debug;
	extern uint64_t cbp_print_access_debug_pc;
	extern uint32_t cbp_print_access_debug_pc_count;
	extern uint32_t cbp_seed;
	extern uint32_t cbp_bloom_filter_size;
	extern bool     cbp_enable_dyn_degree_detector;
	extern bool     cbp_print_trace;
	extern uint32_t cbp_action_tracker_size;
}

uint32_t cbp_debug_print_count = 0;


const char* CBPMapRewardTypeString[] = {"none", "prefetch_queue_hit", "prefetch_queue_unhit"};
const char* CBPgetRewardTypeString(CBPRewardType type)
{
	assert(type < CBPRewardType::cbp_num_rewards);
	return CBPMapRewardTypeString[(uint32_t)type];
}


void CBP_STEntry::update(uint64_t page, uint64_t pc, uint32_t offset, uint64_t address)
{
	assert(this->page == page);

	/* insert PC */
	if(this->pcs.size() >= knob::cbp_max_pcs)
	{
		this->pcs.pop_front();
	}
	...
	if(!this->offsets.empty())
	{
		...
		if(this->deltas.size() >= knob::cbp_max_deltas)
		{
	...
	/* insert offset */
	if(this->offsets.size() >= knob::cbp_max_offsets)
	{
	...
	if(knob::cbp_print_trace)
	{
		fprintf(stdout, "[TRACE] page %16lx| pc %16lx| offset %2u\n", page, pc, offset);
	}
}

uint32_t CBP_STEntry::get_delta_sig()
{
...
uint32_t CBP_STEntry::get_delta_sig2()
{
	uint32_t curr_sig = 0;
...
uint32_t CBP_STEntry::get_pc_sig()
{
	uint32_t signature = 0;
...
uint32_t CBP_STEntry::get_offset_sig()
{
	uint32_t signature = 0;
...
void CBP_STEntry::track_prefetch(uint32_t pred_offset, int32_t pref_offset)
{
...
void CBP_STEntry::insert_action_tracker(int32_t pref_offset)
{
	// bool found = false;
	auto it = find_if(action_tracker.begin(), action_tracker.end(), [pref_offset](CBPActionTracker *at){return at->action == pref_offset;});
	...
	{
		if(action_tracker.size() >= knob::cbp_action_tracker_size)
		{
			CBPActionTracker *victim = action_tracker.front();
			action_tracker.pop_front();
			delete victim;
		}
		action_tracker.push_back(new CBPActionTracker(pref_offset, 0));
	}
}

bool CBP_STEntry::search_action_tracker(uint64_t action, int32_t &conf)
{
	conf = 0;
	auto it = find_if(action_tracker.begin(), action_tracker.end(), [action](CBPActionTracker *at){return at->action == action;});
	if(it != action_tracker.end())
	{
		conf = (*it)->conf;
		return true;
...
void CBPRecorder::record_access(uint64_t pc, uint64_t address, uint64_t page, uint32_t offset, uint8_t bw_level)
{
...
void CBPRecorder::record_trigger_access(uint64_t page, uint64_t pc, uint32_t offset)
{
	unique_trigger_pcs.insert(pc);
}

void CBPRecorder::record_access_knowledge(CBP_STEntry *stentry)
{
...
{
	bool to_print = true;
	if(knob::cbp_print_access_debug_pc)
	{
		if(stentry->unique_pcs.find(knob::cbp_print_access_debug_pc) == stentry->unique_pcs.end())
		{
			to_print = false;
		}
		else
		{
			cbp_debug_print_count++;
			if(cbp_debug_print_count > knob::cbp_print_access_debug_pc_count)
			{
				to_print = false;
			}
		}
