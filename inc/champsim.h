...
#define NVM_CHANNELS 1      // default: assuming one DIMM per one channel 4GB * 1 => 4GB off-chip memory
#define LOG2_NVM_CHANNELS 0
#define NVM_RANKS 8         // 512MB * 8 ranks => 4GB per DIMM
#define LOG2_NVM_RANKS 0
#define NVM_BANKS 8         // 64MB * 8 banks => 512MB per rank
#define LOG2_NVM_BANKS 3
#define NVM_ROWS 65536      // 2KB * 32K rows => 64MB per bank
#define LOG2_NVM_ROWS 16
#define NVM_COLUMNS 128      // 64B * 32 column chunks (Assuming 1B DRAM cell * 8 chips * 8 transactions = 64B size of column chunks) => 2KB per row
#define LOG2_NVM_COLUMNS 7
#define NVM_ROW_SIZE (BLOCK_SIZE * NVM_COLUMNS / 1024) //1 MB

#define NVM_SIZE (NVM_CHANNELS * NVM_RANKS * NVM_BANKS * NVM_ROWS * NVM_ROW_SIZE / 1024) //KB 
#define NVM_PAGES ((NVM_SIZE << 10) >> 2) 
#define NVM_BW_LEVELS 4

#define HMM_NVM_CHANNELS 1      // default: assuming one DIMM per one channel 4GB * 1 => 4GB off-chip memory
#define LOG2_HMM_NVM_CHANNELS 0
#define HMM_NVM_RANKS 8         // 512MB * 8 ranks => 4GB per DIMM
#define LOG2_HMM_NVM_RANKS 0
#define HMM_NVM_BANKS 8         // 64MB * 8 banks => 512MB per rank
#define LOG2_HMM_NVM_BANKS 3
#define HMM_NVM_ROWS 65536      // 2KB * 32K rows => 64MB per bank
#define LOG2_HMM_NVM_ROWS 16
#define HMM_NVM_COLUMNS 128      // 64B * 32 column chunks (Assuming 1B DRAM cell * 8 chips * 8 transactions = 64B size of column chunks) => 2KB per row
#define LOG2_HMM_NVM_COLUMNS 7
#define HMM_NVM_ROW_SIZE (BLOCK_SIZE * HMM_NVM_COLUMNS / 1024) //1 MB

#define HMM_NVM_SIZE (HMM_NVM_CHANNELS * HMM_NVM_RANKS * HMM_NVM_BANKS * HMM_NVM_ROWS * HMM_NVM_ROW_SIZE / 1024) //KB 
#define HMM_NVM_PAGES ((HMM_NVM_SIZE << 10) >> 2) 
#define HMM_NVM_BW_LEVELS 4
