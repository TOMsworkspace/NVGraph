...
#define COUNTER 2048   //输出多少条地址

extern uint32_t SCHEDULING_LATENCY, EXEC_LATENCY;

void print_core_config();

//tpc prefetcher, instruction tain for sub-prefetcher p1

//bool p1_inst_tain(Prefetcher * tpc_prefetcher, uint64_t pc, uint64_t address, uint32_t src_reg, uint32_t dec_reg);
...
class O3_CPU {
  public:
    ...
    uint64_t last_address; //上一次访问的地址/64
    uint64_t last_ip;//
    uint64_t last_offset; //上一次访问的地址页内偏移,用于计算页内delta
    unordered_map<uint64_t,int> address_reference_frequency; //地址重用次数，以cacheline为单位
    unordered_map<long long,int> page_offset_delta_frequency; //delta频率
    int      count; //计数器，统计连续count条指令
    //ofstream fp1,fp2,fp3,fp4,fp5,fp6;
    //ofstream fp3,fp4,fp5,fp6;
    ofstream fp2,fp3;
    ...
    O3_CPU() {
        ...
        //fp1.open("cacheline_address_2048.txt");
        fp2.open("offset.txt");
        
        fp3.open("cacheline_address.txt");
        /*
        fp4.open("addr_binary.txt");
        fp5.open("ip_cacheline_delta.txt");
        fp6.open("addr_cacheline_delta.txt");
        */
        // branch
