#ifndef LEARNING_ENGINE_FEATUREWISE_RLP_H
#define LEARNING_ENGINE_FEATUREWISE_RLP_H

#include <random>
#include "learning_engine_base.h"
#include "feature_knowledge.h"

class LearningEngineFeaturewise_rlp : public LearningEngineBase
{
private:
...
private:
	...
	void adjust_feature_weights(vector<bool> consensus_vec, RLPRewardType reward_type);
	bool do_fallback(State *state);
	void plot_scores();

public:
	LearningEngineFeaturewise_rlp(Prefetcher *p, float alpha, float gamma, float epsilon, uint32_t actions, uint64_t seed, std::string policy, std::string type, bool zero_init);
	~LearningEngineFeaturewise_rlp();
	uint32_t chooseAction(State *state, float &max_to_avg_q_ratio, vector<bool> &consensus_vec);
	void learn(State *state1, uint32_t action1, int32_t reward, State *state2, uint32_t action2, vector<bool> consensus_vec, RLPRewardType reward_type);
	void dump_stats();
};

#endif /* LEARNING_ENGINE_FEATUREWISE_RLP_H */
