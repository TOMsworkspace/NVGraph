...
{
    if(knob::tpc_enable){
        for(uint32_t index = 0; index < L1D.l1d_prefetchers.size(); ++index)
	    {
            if(!L1D.l1d_prefetchers[index]->get_type().compare("tpc")){/* means TPC */
                tpc = (class TPCPrefetcher *)L1D.l1d_prefetchers[index];
            }
        }
    }
    else{
    ...
    while (continue_reading)
    {
        ...
        else
        {
            ...
            { // successfully read the trace
                ...
                //////////*****************************************************************************************
                /*
                if (warmup_complete[cpu] && count < COUNTER){
                    //uint64_t temp=current_instr.ip;
                    //bitset<64> b1(temp);
                    //fp6<<b1<<endl;
                    uint64_t temp1 = current_instr.ip;
                    uint64_t cacheline_ip = temp1 >> 6;
                    long long delta1=cacheline_ip-last_ip;
                    last_ip=cacheline_ip;
                    fp5<<delta1<<endl;
                }*/
    ...
    while (load_issued < LQ_WIDTH)
    {
        if (RTL0[RTL0_head] < LQ_SIZE)
        {
            ...
            if (LQ.entry[lq_index].event_cycle <= current_core_cycle[cpu])
            {
                ...
                if (warmup_complete[cpu] && count < COUNTER)
                {
                    uint64_t temp = data_packet.full_addr;
                    //bitset<64> b2(temp);
                    //fp4<<b2<<endl;
                    uint64_t cacheline_address = temp >> 6;
                    fp3<<LQ.entry[lq_index].ip<<','<<cacheline_address<<endl;

                    last_offset=cacheline_address&0x3f;
                    fp2<<last_offset<<endl;

                    //long long delta=cacheline_address-last_address;
                    //last_address=cacheline_address;
                    //fp6<<delta<<endl;

                    //fp1 << cacheline_address << endl; //cacheline_address_2048

                    //uint64_t cacheline_in_page=cacheline_address%512;
                    //fp5<<cacheline_in_page<<endl;

                    //long long cacheline_in_page_delta=cacheline_in_page-last_offset;
                    //last_offset=cacheline_in_page;
                    //page_offset_delta_frequency[cacheline_in_page_delta]++;
                    //cacheline_in_page_delta_frequency_2048

                    //address_reference_frequency[cacheline_address]++;
                    
                    //long long cacheline_address_delta = cacheline_address - last_address;
                    //last_address = cacheline_address;
                    //fp3 << cacheline_address_delta << endl; //cacheline_address_delta_2048
                    
                    //uint64_t page_address=cacheline_address>>6;
                    //fp2<<page_address<<endl;//page_address_2048
                ...
                if (count == COUNTER)
                {
                    for (auto ie = page_offset_delta_frequency.begin(); ie != page_offset_delta_frequency.end(); ++ie){
                        //fp4 << ie->first << ',' << ie->second << endl;
                    }
...
int O3_CPU::execute_load(uint32_t rob_index, uint32_t lq_index, uint32_t data_index)
{
    ...
    //tcp sub-prefetcher p1 instruction tain
    if(knob::tpc_enable){
        ooo_model_instr * instr = & ROB.entry[rob_index];

        uint64_t pc = instr->ip;
        uint64_t address = instr->instruction_pa >> LOG2_BLOCK_SIZE;

        for (int i = 0; i < NUM_INSTR_SOURCES; i++)
        {
            if (instr->source_registers[i]){
                for(int j = 0; j < NUM_INSTR_DESTINATIONS; j++){
                    if(instr->destination_registers[j]){
                        p1_inst_tain(tpc, pc, address, instr->source_registers[i], instr->destination_registers[j]);
                    }
                }
            }
        }
