...
#include <getopt.h>  //is used to parse command line option arguments
#include "ooo_cpu.h" //out of order cpu
#include "uncore.h"
#include "knobs.h"
#include <fstream>

#define FIXED_FLOAT(x) std::fixed << std::setprecision(5) << (x) //.5f
...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
    ...
    cout << "Core_" << cpu << "_" << cache->NAME << "_total_access " << TOTAL_ACCESS << endl
         ...
         << "Core_" << cpu << "_" << cache->NAME << "_average_miss_latency " << (1.0 * (cache->total_miss_latency)) / (TOTAL_MISS + 1) << endl
         << endl
...
uint64_t va_to_pa(uint32_t cpu, uint64_t instr_id, uint64_t va, uint64_t unique_vpage)
{
    ...
    { // no VA => PA translation found
        ...
        { // not enough memory
            ...
            // here, NVGraph randomly selects a page that is not recently used and we only track 32K recently accessed pages
            uint8_t found_NRU = 0;
        ...
        else
        {
            ...
            // encoding cpu number
            // this allows NVGraph to run homogeneous multi-programmed workloads without VA => PA aliasing
...
void print_knobs()
{
    ...
    cout << "L1 prefetchers: " << endl;
    for(auto pf_type : knob::l1d_prefetcher_types)
        cout << pf_type << " ";
    cout << endl << "L2 prefetchers: " << endl;
    for(auto pf_type : knob::l2c_prefetcher_types)
        cout << pf_type << " ";
    cout << endl << endl;
...
{
    for (uint32_t index = 0; index < NUM_CPUS; ++index)
        generated[index] = false; //多核系统CPU数组
    ...
    cout << "*************************************************" << endl
         << "   NVGraph Multicore Out-of-Order Simulator" << endl
    ...
    if (knob::knob_low_bandwidth){
        ...
        NVM_MTPS = knob::dram_io_freq / 4;
        HMM_NVM_MTPS = knob::dram_io_freq / 4;
    }
    else{
    ...
    // DRAM access latency
    DRAM_tRP = (uint32_t)((1.0 * tRP_DRAM_NANOSECONDS * CPU_FREQ) / 1000);
    DRAM_tRCD = (uint32_t)((1.0 * tRCD_DRAM_NANOSECONDS * CPU_FREQ) / 1000);
    DRAM_tCAS = (uint32_t)((1.0 * tCAS_DRAM_NANOSECONDS * CPU_FREQ) / 1000);
    ...
    NVM_tRP = (uint32_t)((1.0 * tRP_NVM_NANOSECONDS * CPU_FREQ) / 1000);
    NVM_tRCD = (uint32_t)((1.0 * tRCD_NVM_NANOSECONDS * CPU_FREQ) / 1000);
    NVM_tCAS = (uint32_t)((1.0 * tCAS_NVM_NANOSECONDS * CPU_FREQ) / 1000);

    // default: 16 = (64 / 8) * (3200 / 1600)
    // it takes 16 CPU cycles to tranfser 64B cache block on a 8B (64-bit) bus
    // note that dram burst length = BLOCK_SIZE/DRAM_CHANNEL_WIDTH
    NVM_DBUS_RETURN_TIME = (BLOCK_SIZE / NVM_CHANNEL_WIDTH) * (1.0 * CPU_FREQ / NVM_MTPS);
    NVM_DBUS_MAX_CAS = NVM_CHANNELS * (knob::measure_dram_bw_epoch / NVM_DBUS_RETURN_TIME);
    

    HMM_NVM_tRP = (uint32_t)((1.0 * tRP_HMM_NVM_NANOSECONDS * CPU_FREQ) / 1000);
    HMM_NVM_tRCD = (uint32_t)((1.0 * tRCD_HMM_NVM_NANOSECONDS * CPU_FREQ) / 1000);
    HMM_NVM_tCAS = (uint32_t)((1.0 * tCAS_HMM_NVM_NANOSECONDS * CPU_FREQ) / 1000);

    // default: 16 = (64 / 8) * (3200 / 1600)
    // it takes 16 CPU cycles to tranfser 64B cache block on a 8B (64-bit) bus
    // note that dram burst length = BLOCK_SIZE/DRAM_CHANNEL_WIDTH
    HMM_NVM_DBUS_RETURN_TIME = (BLOCK_SIZE / HMM_NVM_CHANNEL_WIDTH) * (1.0 * CPU_FREQ / HMM_NVM_MTPS);
    HMM_NVM_DBUS_MAX_CAS = HMM_NVM_CHANNELS * (knob::measure_dram_bw_epoch / HMM_NVM_DBUS_RETURN_TIME);
    ...
    for (int i = 0; i < argc; i++)
    {
        if (found_traces)
        {
            ...
            {
                cout << "NVGraph does not support traces other than gz or xz compression!" << endl;
                assert(0);
            }
    ...
    uncore.LLC.llc_initialize_replacement(); //空函数？
    uncore.LLC.llc_prefetcher_initialize();  //空函数？
        ...
        for (int index = 0; index < NUM_CPUS; ++index)
        {
            ...
            if (knob::measure_ipc && current_core_cycle[i] >= ooo_cpu[i].next_measure_ipc_cycle)
            {
                ...
                if (ins_in_epoch >= ooo_cpu[i].last_ins_in_epoch)
                {
                    ...
                    ooo_cpu[i].broadcast_ipc(1); //递归到底也是空函数？
                }
                else
                {
                    /* IPC decreased */
            ...
            { //阻塞周期＞当前周期时，不能执行任何指令
                ...
                // execute
                ooo_cpu[i].execute_instruction(); //只执行非内存相关指令

                // memory operation
                ooo_cpu[i].schedule_memory_instruction(); //内存相关指令
    ...
    cout << endl
         << "NVGraph completed all CPUs" << endl;
    if (NUM_CPUS > 1)
    {
