#include "hybrid_memory_controller.h"

// initialized in main.cc
uint32_t HMM_NVM_MTPS, HMM_NVM_DBUS_RETURN_TIME, HMM_NVM_DBUS_MAX_CAS,
         HMM_NVM_tRP, HMM_NVM_tRCD, HMM_NVM_tCAS;

void print_hybrid_memory_config()
{
    cout << "hmm_nvm_channel_width " << HMM_NVM_CHANNEL_WIDTH << endl
        << "hmm_nvm_wq_size " << HMM_NVM_WQ_SIZE << endl
        << "hmm_nvm_rq_size " << HMM_NVM_RQ_SIZE << endl
        << "hmm_nvm_tRP " << tRP_HMM_NVM_NANOSECONDS << endl
        << "hmm_nvm_tRCD " << tRCD_HMM_NVM_NANOSECONDS << endl
        << "hmm_nvm_tCAS " << tCAS_HMM_NVM_NANOSECONDS << endl
        << "hmm_nvm_dbus_turn_around_time " << HMM_NVM_DBUS_TURN_AROUND_TIME << endl
        << "hmm_nvm_write_high_wm " << HMM_NVM_WRITE_HIGH_WM << endl
        << "hmm_nvm_write_low_wm " << HMM_NVM_WRITE_LOW_WM << endl
        << "min_hmm_nvm_writes_per_switch " << MIN_HMM_NVM_WRITES_PER_SWITCH << endl
        << "hmm_nvm_mtps " << HMM_NVM_MTPS << endl
        << "hmm_nvm_dbus_return_time " << HMM_NVM_DBUS_RETURN_TIME << endl
        << endl;
}

void HYBRID_MEMORY_CONTROLLER::reset_remain_requests(PACKET_QUEUE *queue, uint32_t channel)
{
    for (uint32_t i=0; i<queue->SIZE; i++) {
        if (queue->entry[i].scheduled) {
            ...
            uint32_t op_cpu = queue->entry[i].cpu,
                     op_channel = nvm_get_channel(op_addr), 
                     op_rank = nvm_get_rank(op_addr), 
                     op_bank = nvm_get_bank(op_addr), 
                     op_row = nvm_get_row(op_addr);

#ifdef DEBUG_PRINT
            //uint32_t op_column = nvm_get_column(op_addr);
#endif

            // update open row
            if ((bank_request[op_channel][op_rank][op_bank].cycle_available - HMM_NVM_tCAS) <= current_core_cycle[op_cpu])
                bank_request[op_channel][op_rank][op_bank].open_row = op_row;
            else
                bank_request[op_channel][op_rank][op_bank].open_row = UINT32_MAX;

            // this bank is ready for another NVM request
...
void HYBRID_MEMORY_CONTROLLER::operate()
{
    for (uint32_t i=0; i<HMM_NVM_CHANNELS; i++) {
        //if ((write_mode[i] == 0) && (WQ[i].occupancy >= HMM_NVM_WRITE_HIGH_WM)) {
      if ((write_mode[i] == 0) && ((WQ[i].occupancy >= HMM_NVM_WRITE_HIGH_WM) || ((RQ[i].occupancy == 0) && (WQ[i].occupancy > 0)))) { // use idle cycles to perform writes
            write_mode[i] = 1;

            // reset scheduled RQ requests
            reset_remain_requests(&RQ[i], i);
            // add data bus turn-around time
            dbus_cycle_available[i] += HMM_NVM_DBUS_TURN_AROUND_TIME;
        } else if (write_mode[i]) {

            if (WQ[i].occupancy == 0)
                write_mode[i] = 0;
            else if (RQ[i].occupancy && (WQ[i].occupancy < HMM_NVM_WRITE_LOW_WM))
                write_mode[i] = 0;
...
void HYBRID_MEMORY_CONTROLLER::schedule(PACKET_QUEUE *queue)
{
    uint64_t read_addr;
    ...
    for (uint32_t i=0; i<queue->SIZE; i++) {
        ...
        read_channel = nvm_get_channel(read_addr);
        read_rank = nvm_get_rank(read_addr);
        read_bank = nvm_get_bank(read_addr);
        ...
        read_row = nvm_get_row(read_addr);
        //read_column = nvm_get_column(read_addr);
    ...
    if (oldest_index != -1) { // scheduler might not find anything if all requests are already scheduled or all banks are busy
        ...
        uint64_t LATENCY = 0;
        if (row_buffer_hit)  
            LATENCY = HMM_NVM_tCAS;
        else 
            LATENCY = HMM_NVM_tRP + HMM_NVM_tRCD + HMM_NVM_tCAS;
...
void HYBRID_MEMORY_CONTROLLER::process(PACKET_QUEUE *queue)
{
    ...
    uint32_t op_cpu = queue->entry[request_index].cpu,
             ...
             op_bank = nvm_get_bank(op_addr);
#ifdef DEBUG_PRINT
    uint32_t op_row = nvm_get_row(op_addr), 
             op_column = nvm_get_column(op_addr);
#endif
    ...
    // paid all NVM access latency, data is ready to be processed
    if (bank_request[op_channel][op_rank][op_bank].cycle_available <= current_core_cycle[op_cpu]) {
        ...
        if (dbus_cycle_available[op_channel] <= current_core_cycle[op_cpu]) {
            ...
            if (queue->is_WQ) {
                ...
                dbus_cycle_available[op_channel] = current_core_cycle[op_cpu] + HMM_NVM_DBUS_RETURN_TIME;
...
int HYBRID_MEMORY_CONTROLLER::add_rq(PACKET *packet)
{
    ...
    uint32_t channel = nvm_get_channel(packet->address);
    int wq_index = check_nvm_queue(&WQ[channel], packet);
    if (wq_index != -1) {
    ...
    int index = check_nvm_queue(&RQ[channel], packet);
    if (index != -1)
        return index; // merged index

    // search for the empty index
    for (index=0; index<HMM_NVM_RQ_SIZE; index++) {
...
#ifdef DEBUG_PRINT
            uint32_t channel = nvm_get_channel(packet->address),
                     rank = nvm_get_rank(packet->address),
                     bank = nvm_get_bank(packet->address),
                     row = nvm_get_row(packet->address),
                     column = nvm_get_column(packet->address); 
#endif
...
int HYBRID_MEMORY_CONTROLLER::add_wq(PACKET *packet)
{
    ...
    uint32_t channel = nvm_get_channel(packet->address);
    int index = check_nvm_queue(&WQ[channel], packet);
    if (index != -1)
        return index; // merged index

    // search for the empty index
    for (index=0; index<HMM_NVM_WQ_SIZE; index++) {
...
int HYBRID_MEMORY_CONTROLLER::add_pq(PACKET *packet)
{
    return -1;
}

void HYBRID_MEMORY_CONTROLLER::return_data(PACKET *packet)
{

}

void HYBRID_MEMORY_CONTROLLER::update_schedule_cycle(PACKET_QUEUE *queue)
{
...
void HYBRID_MEMORY_CONTROLLER::update_process_cycle(PACKET_QUEUE *queue)
{
...
int HYBRID_MEMORY_CONTROLLER::check_nvm_queue(PACKET_QUEUE *queue, PACKET *packet)
{
    // search write queue
...
uint32_t HYBRID_MEMORY_CONTROLLER::nvm_get_channel(uint64_t address)
{
    if (LOG2_HMM_NVM_CHANNELS == 0)
        return 0;

    int shift = 0;

    return (uint32_t) (address >> shift) & (HMM_NVM_CHANNELS - 1);
}

uint32_t HYBRID_MEMORY_CONTROLLER::nvm_get_bank(uint64_t address)
{
    if (LOG2_HMM_NVM_BANKS == 0)
        return 0;

    int shift = LOG2_HMM_NVM_CHANNELS;

    return (uint32_t) (address >> shift) & (HMM_NVM_BANKS - 1);
}

uint32_t HYBRID_MEMORY_CONTROLLER::nvm_get_column(uint64_t address)
{
    if (LOG2_HMM_NVM_COLUMNS == 0)
        return 0;

    int shift = LOG2_HMM_NVM_BANKS + LOG2_HMM_NVM_CHANNELS;

    return (uint32_t) (address >> shift) & (HMM_NVM_COLUMNS - 1);
}

uint32_t HYBRID_MEMORY_CONTROLLER::nvm_get_rank(uint64_t address)
{
    if (LOG2_HMM_NVM_RANKS == 0)
        return 0;

    int shift = LOG2_HMM_NVM_COLUMNS + LOG2_HMM_NVM_BANKS + LOG2_HMM_NVM_CHANNELS;

    return (uint32_t) (address >> shift) & (HMM_NVM_RANKS - 1);
}

uint32_t HYBRID_MEMORY_CONTROLLER::nvm_get_row(uint64_t address)
{
    if (LOG2_HMM_NVM_ROWS == 0)
        return 0;

    int shift = LOG2_HMM_NVM_RANKS + LOG2_HMM_NVM_COLUMNS + LOG2_HMM_NVM_BANKS + LOG2_HMM_NVM_CHANNELS;

    return (uint32_t) (address >> shift) & (HMM_NVM_ROWS - 1);
}

uint32_t HYBRID_MEMORY_CONTROLLER::get_occupancy(uint8_t queue_type, uint64_t address)
{
    uint32_t channel = nvm_get_channel(address);
    if (queue_type == 1)
...
uint32_t HYBRID_MEMORY_CONTROLLER::get_size(uint8_t queue_type, uint64_t address)
{
    uint32_t channel = nvm_get_channel(address);
    if (queue_type == 1)
...
void HYBRID_MEMORY_CONTROLLER::increment_WQ_FULL(uint64_t address)
{
    uint32_t channel = nvm_get_channel(address);
    WQ[channel].FULL++;
}
