...
uint32_t NVM_MTPS, NVM_DBUS_RETURN_TIME, NVM_DBUS_MAX_CAS,
         NVM_tRP, NVM_tRCD, NVM_tCAS;

void print_nvm_config()
{
    cout << "nvm_channel_width " << NVM_CHANNEL_WIDTH << endl
        << "nvm_wq_size " << NVM_WQ_SIZE << endl
        << "nvm_rq_size " << NVM_RQ_SIZE << endl
        << "nvm_tRP " << tRP_NVM_NANOSECONDS << endl
        << "nvm_tRCD " << tRCD_NVM_NANOSECONDS << endl
        << "nvm_tCAS " << tCAS_NVM_NANOSECONDS << endl
        << "nvm_dbus_turn_around_time " << NVM_DBUS_TURN_AROUND_TIME << endl
        << "nvm_write_high_wm " << NVM_WRITE_HIGH_WM << endl
        << "nvm_write_low_wm " << NVM_WRITE_LOW_WM << endl
        << "min_nvm_writes_per_switch " << MIN_NVM_WRITES_PER_SWITCH << endl
        << "nvm_mtps " << NVM_MTPS << endl
        << "nvm_dbus_return_time " << NVM_DBUS_RETURN_TIME << endl
        << endl;
}

void NVM_MEMORY_CONTROLLER::reset_remain_requests(PACKET_QUEUE *queue, uint32_t channel)
{
    for (uint32_t i=0; i<queue->SIZE; i++) {
        if (queue->entry[i].scheduled) {
            ...
            // update open row
            if ((bank_request[op_channel][op_rank][op_bank].cycle_available - NVM_tCAS) <= current_core_cycle[op_cpu])
...
void NVM_MEMORY_CONTROLLER::operate()
{
    for (uint32_t i=0; i<NVM_CHANNELS; i++) {
        //if ((write_mode[i] == 0) && (WQ[i].occupancy >= NVM_WRITE_HIGH_WM)) {
      if ((write_mode[i] == 0) && ((WQ[i].occupancy >= NVM_WRITE_HIGH_WM) || ((RQ[i].occupancy == 0) && (WQ[i].occupancy > 0)))) { // use idle cycles to perform writes
            write_mode[i] = 1;

            // reset scheduled RQ requests
            reset_remain_requests(&RQ[i], i);
            // add data bus turn-around time
            dbus_cycle_available[i] += NVM_DBUS_TURN_AROUND_TIME;
        } else if (write_mode[i]) {

            if (WQ[i].occupancy == 0)
                write_mode[i] = 0;
            else if (RQ[i].occupancy && (WQ[i].occupancy < NVM_WRITE_LOW_WM))
                write_mode[i] = 0;
...
void NVM_MEMORY_CONTROLLER::schedule(PACKET_QUEUE *queue)
{
    uint64_t read_addr;
    ...
    if (oldest_index != -1) { // scheduler might not find anything if all requests are already scheduled or all banks are busy
        ...
        if (row_buffer_hit)  
            ...
            LATENCY = NVM_tRP + NVM_tRCD + NVM_tCAS;
...
void NVM_MEMORY_CONTROLLER::process(PACKET_QUEUE *queue)
{
    ...
    if (bank_request[op_channel][op_rank][op_bank].cycle_available <= current_core_cycle[op_cpu]) {
        ...
        if (dbus_cycle_available[op_channel] <= current_core_cycle[op_cpu]) {
            ...
            if (queue->is_WQ) {
                ...
                dbus_cycle_available[op_channel] = current_core_cycle[op_cpu] + NVM_DBUS_RETURN_TIME;
...
int NVM_MEMORY_CONTROLLER::add_rq(PACKET *packet)
{
    ...
    for (index=0; index<NVM_RQ_SIZE; index++) {
...
int NVM_MEMORY_CONTROLLER::add_wq(PACKET *packet)
{
    ...
    // search for the empty index
    for (index=0; index<NVM_WQ_SIZE; index++) {
...
int NVM_MEMORY_CONTROLLER::add_pq(PACKET *packet)
{
    return -1;
}

void NVM_MEMORY_CONTROLLER::return_data(PACKET *packet)
{

}

void NVM_MEMORY_CONTROLLER::update_schedule_cycle(PACKET_QUEUE *queue)
{
...
void NVM_MEMORY_CONTROLLER::update_process_cycle(PACKET_QUEUE *queue)
{
...
int NVM_MEMORY_CONTROLLER::check_nvm_queue(PACKET_QUEUE *queue, PACKET *packet)
{
    // search write queue
...
uint32_t NVM_MEMORY_CONTROLLER::nvm_get_channel(uint64_t address)
{
    if (LOG2_NVM_CHANNELS == 0)
        return 0;

    int shift = 0;

    return (uint32_t) (address >> shift) & (NVM_CHANNELS - 1);
}

uint32_t NVM_MEMORY_CONTROLLER::nvm_get_bank(uint64_t address)
{
    if (LOG2_NVM_BANKS == 0)
        return 0;

    int shift = LOG2_NVM_CHANNELS;

    return (uint32_t) (address >> shift) & (NVM_BANKS - 1);
}

uint32_t NVM_MEMORY_CONTROLLER::nvm_get_column(uint64_t address)
{
    if (LOG2_NVM_COLUMNS == 0)
        return 0;

    int shift = LOG2_NVM_BANKS + LOG2_NVM_CHANNELS;

    return (uint32_t) (address >> shift) & (NVM_COLUMNS - 1);
}

uint32_t NVM_MEMORY_CONTROLLER::nvm_get_rank(uint64_t address)
{
    if (LOG2_NVM_RANKS == 0)
        return 0;

    int shift = LOG2_NVM_COLUMNS + LOG2_NVM_BANKS + LOG2_NVM_CHANNELS;

    return (uint32_t) (address >> shift) & (NVM_RANKS - 1);
}

uint32_t NVM_MEMORY_CONTROLLER::nvm_get_row(uint64_t address)
{
    if (LOG2_NVM_ROWS == 0)
        return 0;

    int shift = LOG2_NVM_RANKS + LOG2_NVM_COLUMNS + LOG2_NVM_BANKS + LOG2_NVM_CHANNELS;

    return (uint32_t) (address >> shift) & (NVM_ROWS - 1);
}

uint32_t NVM_MEMORY_CONTROLLER::get_occupancy(uint8_t queue_type, uint64_t address)
{
...
uint32_t NVM_MEMORY_CONTROLLER::get_size(uint8_t queue_type, uint64_t address)
{
    uint32_t channel = nvm_get_channel(address);
    if (queue_type == 1)
...
void NVM_MEMORY_CONTROLLER::increment_WQ_FULL(uint64_t address)
{
    uint32_t channel = nvm_get_channel(address);
    WQ[channel].FULL++;
}
